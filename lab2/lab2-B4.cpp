...
mat4 T_bones[kMaxBones];
mat4 R_bones[kMaxBones];
mat4 M_bones[kMaxBones];
mat4 M_bones_prime[kMaxBones];
mat4 M_mb[kMaxBones];
mat4 M_bm[kMaxBones];
...
void createMatricesForBones() {
  ...
  for (int i = 0; i < kMaxBones; i++) {
    // M_bones_inverse
    M_bones[i] = T_bones[i] * g_bones[i].rot;

    // M_bones_prime
    M_bones_prime[i] = Mult(T_bones[i], g_bonesRes[i].rot);

    // M_mb
    M_mb[i] = IdentityMatrix();
    for (int j = i; j >= 0; j--) {
      M_mb[i] = M_mb[i] * inverse(M_bones[j]);
    }

    // M_bm
    M_bm[i] = IdentityMatrix();
    for (int j = i; j < kMaxBones; j++) {
      M_bm[i] = M_bm[i] * M_bones_prime[j];
    }
  }
}
...
void DeformCylinder() {
  ...
  createMatricesForBones();

  // for all vertices
  for (row = 0; row < kMaxRow; row++) {
    for (corner = 0; corner < kMaxCorners; corner++) {
      g_vertsRes[row][corner] = g_normalsOrg[row][corner];
      // ---------=========  PART 4 ===========---------
      ...
      for (int bone = 0; bone < kMaxBones; bone++) {
        sum = sum + g_boneWeights[row][corner][bone] * M_bm[bone] * M_mb[bone] * g_vertsOrg[row][corner];
      }
